    ...
    <nav className='lg:hidden block z-50 py-[12px]'>
        <div className="container">
            <div id="NavBar_Row" className='py-[20px] flex items-center justify-between'>
                ...
                {/* -----------Nav button--------  */}
                <div className='flex items-center gap-5 text-2xl text-borderCol' data-aos="fade-down" data-aos-easing="ease-in-sine" data-aos-delay="1400">
        ...
        <div className={`absolute top-0 left-0 w-full h-screen duration-[.3s]  ${Value? 'z-[-10]' : 'z-[40]'}`}>
            ...
            <div className={`h-screen w-[60%] absolute top-0 bg-primary p-5 duration-[.8s] shadow-[0_3px_10px_rgb(0,0,0,0.2)] overflow-hidden z-50 ${Value? ' translate-x-[-520px] md:translate-x-[-740px] ' : ' translate-x-[-0px]'}`}>
                ...
                <ul className='flex flex-col items-start gap-5 text-xl mt-25'>
                    ...
                    <span className='w-full h-[1px] bg-gray-400'></span>
                    <li><Link onClick={()=>SetValue(!Value)} className='text-[#F8F8F8] font-manrope font-medium flex items-center gap-2' to={'/contact'}><FiPhoneCall/> Contact Us</Link></li>
                </ul>
            </div>
        </div>
    </nav>
